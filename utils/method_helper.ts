...
export function print_java_method_params(args: any[]) {
  var params = "";
  for (var idx = 0; idx < args.length; idx++) {
    params += `\nparam ${idx}: ${args[idx]}`;
  }
...
export function hook_methods_by_class_name(clz: string) {
...
export function hook_methods_by_class(target_clz: Java.Wrapper) {
  const methods = target_clz.class.getDeclaredMethods();
  for (var idx = 0; idx < methods.length - 1; idx++) {
    let method_name = methods[idx].getName();
    try {
